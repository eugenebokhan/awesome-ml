...
class FNSUdnieInput : MLFeatureProvider {
...
@available(macOS 10.13, iOS 11.0, tvOS 11.0, watchOS 4.0, *)
class FNSUdnieOutput : MLFeatureProvider {
...
class FNSUdnie {
    ...
    convenience init() {
        let bundle = Bundle(for: FNSUdnie.self)
        let assetPath = bundle.url(forResource: "FNSUdnie", withExtension:"mlmodelc")
    ...
    /**
     ...
     - parameters:
     - input: the input to the prediction as FNSUdnieInput
     - throws: an NSError object that describes the problem
     - returns: the result of the prediction as FNSUdnieOutput
     */
    func prediction(input: FNSUdnieInput) throws -> FNSUdnieOutput {
        let outFeatures = try model.prediction(from: input)
        let result = FNSUdnieOutput(outputImage: outFeatures.featureValue(for: "outputImage")!.imageBufferValue!)
        return result
    }
    ...
    /**
     ...
     - returns: the result of the prediction as FNSUdnieOutput
     */
    func prediction(inputImage: CVPixelBuffer) throws -> FNSUdnieOutput {
        let input_ = FNSUdnieInput(inputImage: inputImage)
