...
class FNSWaveInput : MLFeatureProvider {
...
@available(macOS 10.13, iOS 11.0, tvOS 11.0, watchOS 4.0, *)
class FNSWaveOutput : MLFeatureProvider {
...
class FNSWave {
    ...
    convenience init() {
        let bundle = Bundle(for: FNSWave.self)
        let assetPath = bundle.url(forResource: "wave", withExtension:"mlmodelc")
    ...
    /**
     ...
     - parameters:
     - input: the input to the prediction as waveInput
     - throws: an NSError object that describes the problem
     - returns: the result of the prediction as waveOutput
     */
    func prediction(input: FNSWaveInput) throws -> FNSWaveOutput {
        let outFeatures = try model.prediction(from: input)
        let result = FNSWaveOutput(add_37__0: outFeatures.featureValue(for: "add_37__0")!.imageBufferValue!)
        return result
    }
    ...
    /**
     ...
     - returns: the result of the prediction as waveOutput
     */
    func prediction(img_placeholder__0: CVPixelBuffer) throws -> FNSWaveOutput {
        let input_ = FNSWaveInput(img_placeholder__0: img_placeholder__0)
