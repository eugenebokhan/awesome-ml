...
class FNSCandyInput : MLFeatureProvider {
...
@available(macOS 10.13, iOS 11.0, tvOS 11.0, watchOS 4.0, *)
class FNSCandyOutput : MLFeatureProvider {
...
class FNSCandy {
    ...
    convenience init() {
        let bundle = Bundle(for: FNSCandy.self)
        let assetPath = bundle.url(forResource: "FNSCandy", withExtension:"mlmodelc")
    ...
    /**
     ...
     - parameters:
     - input: the input to the prediction as FNSCandyInput
     - throws: an NSError object that describes the problem
     - returns: the result of the prediction as FNSCandyOutput
     */
    func prediction(input: FNSCandyInput) throws -> FNSCandyOutput {
        let outFeatures = try model.prediction(from: input)
        let result = FNSCandyOutput(outputImage: outFeatures.featureValue(for: "outputImage")!.imageBufferValue!)
        return result
    }
    ...
    /**
     ...
     - returns: the result of the prediction as FNSCandyOutput
     */
    func prediction(inputImage: CVPixelBuffer) throws -> FNSCandyOutput {
        let input_ = FNSCandyInput(inputImage: inputImage)
