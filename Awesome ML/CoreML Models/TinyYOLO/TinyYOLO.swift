...
class TinyYOLOInput : MLFeatureProvider {
...
class TinyYOLOOutput : MLFeatureProvider {
    ...
    /// The 13x13 grid with the bounding box data as 125 x 13 x 13 3-dimensional array of doubles
...
class TinyYOLO {
    ...
    public let maxBoundingBoxes = 10
    ...
    /**
     ...
     - parameters:
     - input: the input to the prediction as TinyYOLOInput
     - throws: an NSError object that describes the problem
     - returns: the result of the prediction as TinyYOLOOutput
     */
    func prediction(input: TinyYOLOInput) throws -> TinyYOLOOutput {
        let outFeatures = try model.prediction(from: input)
        let result = TinyYOLOOutput(grid: outFeatures.featureValue(for: "grid")!.multiArrayValue!)
        return result
    }
    ...
    /**
     ...
     - returns: the result of the prediction as TinyYOLOOutput
     */
    func prediction(image: CVPixelBuffer) throws -> TinyYOLOOutput {
        let input_ = TinyYOLOInput(image: image)
    ...
    public func predict(image: CVPixelBuffer) throws -> [Prediction] {
        if let output = try? prediction(image: image) {
